class GuiBase {
    ...
    __New(container, themeObj, config) {
        ...
        ; TODO: Untangle needing an app in a GUI window to avoid a circular dependency
    ...
    __Delete() {
        if (this.container) {
            this.container["manager.event"].Unregister(Events.MOUSE_MOVE, "Gui" . this.guiId)
            this.container["manager.event"].Unregister(Events.WM_NCCALCSIZE, "Gui" . this.guiId)
            this.container["manager.event"].Unregister(Events.WM_NCACTIVATE, "Gui" . this.guiId)
            this.container["manager.event"].Unregister(Events.WM_NCHITTEST, "Gui" . this.guiId)
        }
    ...
    */
    ...
    Close(submit := false) {
        ...
        if (!this.isClosed && WinExist("ahk_id " . this.guiObj.Hwnd)) {
            this.container["manager.gui"].StoreWindowState(this)
            WinClose("ahk_id " . this.guiObj.Hwnd)
        } else {
            this.Destroy()
        }

        this.container["manager.gui"].CleanupWindow(this.guiId)
    }
    ...
    Destroy() {
        ...
        if (this.owner) {
            this.container["manager.gui"].ReleaseFromParent(this.guiId)
        }
    ...
    Cleanup() {
        this.container["manager.gui"].UnloadComponent(this.guiId)
